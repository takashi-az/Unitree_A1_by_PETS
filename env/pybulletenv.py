...
class pybulletenv():
    ...
    def do_simulation(self,action,obs,frame_skip):
    ...
    def obs_fit_in_limit(self,obs):
        ...
        limit_low = -np.array([np.pi/9,np.pi/9*2,np.pi/3*2 , np.pi/18,np.pi/9*2,np.pi/3*2 , np.pi/9,np.pi/9*2,np.pi/3*2 , np.pi/18,np.pi/9*2,np.pi/3*2])
        for j in range(self.movale_joints):
    ...
    def _get_obs(self):
        ...
        done = abs(curpos[3]) > np.pi/3 or abs(curpos[4]) > np.pi/3 or abs(curpos[5]) > np.pi/2 or done
        curpos = curpos[2:]        
