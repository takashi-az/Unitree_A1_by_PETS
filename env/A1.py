...
class Env(pybulletenv):
    def __init__(self):
        ...
        self.action_high = 0.3*np.ones(12)
        self.action_low = -0.3*np.ones(12)
        self.frame_skip = 50
    ...
    def step(self, action, obs):
        ...
        reward = -(np.where(obs[4]>0.1,-5*obs[4],-3*(obs[4]-0.1))\
                + np.where(np.abs(obs[3])>np.pi/9*2,np.abs(obs[3])-np.pi/9*2,0) \
                + np.where(np.abs(obs[2])>np.pi/9*2,2*(np.abs(obs[2])-np.pi/9*2),0) \
                + np.where(np.abs(obs[8])>1,2*(np.abs(obs[8])-1),0) \
                + np.where(np.abs(obs[1])>np.pi/9*2,2*(np.abs(obs[1])-np.pi/9*2),0))
