...
class MPC(Controller):
    ...
    def __init__(self, params):
        ...
        self.dO, self.dU = params.env.observation_shape, params.env.action_shape
        self.ac_ub, self.ac_lb = params.env.action_high, params.env.action_low
        ...
        self.ign_var = params.prop_cfg.get("ign_var", True) or self.prop_mode == "E"
        ...
        self.save_all_models = params.log_cfg.get("save_all_models", True)
        ...
        self.writer = SummaryWriter('loss/{}'.format(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")))

    def train(self, obs_trajs, acs_trajs, rews_trajs,iter):
        ...
        for obs, acs in zip(obs_trajs, acs_trajs):
            ...
            new_train_targs.append(obs[1:])

        self.train_in = np.concatenate([self.train_in] + new_train_in, axis=0)
        self.train_targs = np.concatenate([self.train_targs] + new_train_targs, axis=0)

        if self.train_in.shape[0] > 60000:
            self.train_in = self.train_in[-60000:]
            self.train_targs = self.train_targs[-60000:]
        ...
        for _ in epoch_range:
            ...
            mse = ((mean - val_targ) ** 2).mean(0).mean(0)/5
            body_z_mse = mse[0]
            body_roll_mse,body_pitch_mse,body_yaw_mse = mse[1],mse[2],mse[3]            

            epoch_range.set_postfix({
                "Training loss(es)": mse_losses.detach().cpu().numpy()
            })
            self.writer.add_scalar('loss_mse/model', mse_losses.sum()/5 ,iter)
            # self.writer.add_scalar('body_x/model', body_x_mse ,iter)
            # self.writer.add_scalar('body_y/model', body_y_mse ,iter)
            self.writer.add_scalar('body_z/model', body_z_mse ,iter)
            self.writer.add_scalar('body_roll/model', body_roll_mse ,iter)
            self.writer.add_scalar('body_pitch/model', body_pitch_mse ,iter)
            self.writer.add_scalar('body_yaw/model', body_yaw_mse ,iter)
