...
class MBExperiment:
    ...
    def run_experiment(self):
        ...
        # print(traj_obs[0].shape)
        # print('traj_acs',traj_acs)
        # print(traj_acs[0].shape)
        # print('traj_rets',traj_rets)
        # # print(traj_rets[0].shape)
        # print('traj_rews',traj_rews)
        # print(traj_rews[0].shape)
        ...
        while 1:
            self.env.comp = 0
            print('0:finish   1:continue')
            ans = int(input())
            if ans:
                t += 1
                self.agent.sample(self.task_hor,self.policy)
            else:
                print('成功回数:{}/{}'.format(self.env.comp,t))
                print('0:not save model, 1:save model')
                ans = int(input())
                if ans:
                    torch.save(self.policy.model.state_dict(),os.path.join(self.logdir,'model_weight.pth'))
                break
